...
Copy the public/ directory (or specific files: assets/js/csv-grid.js, api/data.php, data/customer.db, data/data.csv, index.html) to the production server’s web root (e.g., /var/www/html/ for Apache/Nginx).
...
<csv-grid data-src="/api/data.php" rows-per-page="10">: Displays a grid with records from public/data/customer.db (SQLite, default) or public/data/data.csv (CSV, with ?source=csv).
...
The data source (api/data.php) queries public/data/customer.db (SQLite, default) or public/data/data.csv (CSV, with ?source=csv) using PDO for SQLite.
